...
- **Operational since:** June 2023 (v1.0); current version v2.1 (July 2025); **v2.2 scheduled October 13, 2026**
- **UFS components used:** FV3 atmosphere, MOM6 and HYCOM ocean (configuration-dependent; see note), WAVEWATCH III waves, Noah-MP land, CMEPS coupler
- **Replaced:** [HWRF](./models/tropical_cyclone_models/hwrf.md) and [HMON](./models/tropical_cyclone_models/hmon.md) (both retired 2023)
- **v2.2 (SCN 26-76, September 9, 2026):** Model code resynced to the **February 12, 2026 UFS revision** — the first resync since the July 2024 revision used in v2.1, and the main reason this upgrade matters for the UFS index. Physics moves to a scale-aware 3D-TKE EDMF PBL scheme, Noah-MP land surface, and **RRTMGP radiation replacing RRTMG**, with PBL mixing length varying with wind speed and PBL height. The MOM6 and WW3 submodules are both updated. Vortex initialization and DA are refined (warm-cycling threshold, storm perturbation smoothing, GSI submodule synced to February 4, 2026, adjusted IAU window, `hafs_datool` fixes, TDR superobbing), and two new observation types are assimilated: commercial GPS radio occultation and sUAS data. Post-processing gets an updated GFDL hurricane tracker. One product change: a **10 m wind gust** variable added to the GRIB2 output.
...
- **Open question — HFSA ocean component:** SCN 26-76 lists the ocean update only as "latest MOM6 submodule", implying both configurations are on MOM6, but v2.1 was documented as HFSA on HYCOM (initialized from RTOFS v2.5) and HFSB on MOM6. The published v2.2 sample files carry `hfsa.mom6.*.nc` — as did the v2.1 samples, so the samples do not settle it. The [HAFS entry](./models/tropical_cyclone_models/hafs.md) still describes HFSA as HYCOM-based; this needs verification against operational output before either file is revised, and it bears on the RTOFS → HAFS dependency described under *Models that consume UFS outputs*.
...
- **Ensemble member output — public since September 9, 2026.** SCN 26-48 AAD documents the
  five members for the first time and they appeared the same day, the first object landing
  at 13:40:14 UTC under `s3://noaa-rrfs-ops-pds/rrfsens.YYYYMMDD/CC/m00#/`. This reverses
  the earlier position, taken from SCN 26-48 AAC, that members were produced but not
  ...
  documentation catching up, not a new stream. See the
...
| NARRE (not in repo) | REFS | NARRE's hourly 12-h ensemble guidance replaced by REFS's 60-h forecasts. **Not named in SCN 26-47** — carried here from PNS 25-41; signalled but not formally scheduled |
