...
HARMONIE-DMI output also provides atmospheric forcing for DMI's downstream marine modelling chain — the [**DKSS**](../../../storm_surge_models/regional/denmark/dkss.md) storm-surge model and the [**WAM**](../../../wave_models/regional/denmark/wam-dmi.md) wave model.
...
## Notes
- **DINI is operationally a continuous ensemble (DINI-EPS), not a pure deterministic run.** The UWC-West DINI production is a 1+5 continuous EPS updated hourly; DMI's open-data deterministic DINI product is the control member, collected and distributed every third hour. DMI **also distributes DINI-EPS derived products** through the Forecast EDR API — `harmonie_dini_eps_means`, `harmonie_dini_eps_percentiles`, and `harmonie_dini_eps_probabilities`. These ensemble products are out of scope for this deterministic entry and are documented separately as [HARMONIE-AROME DINI-EPS derived products (DMI)](../../../ensemble_models/regional/denmark/harmonie-dini-eps-dmi.md). Note that they are distributed on the **native 1906 × 1606 / 2 km Lambert grid**, and that DMI publishes no raw ensemble members — only means, percentiles, and threshold probabilities.
...
- **HARMONIE DINI is the meteorological driver for DMI's marine chain.** Both downstream models are documented separately: [DKSS](../../../storm_surge_models/regional/denmark/dkss.md) (storm surge / 3D ocean, HBM-based) and [WAM](../../../wave_models/regional/denmark/wam-dmi.md) (waves, WAM Cy4.5). Both take HARMONIE DINI forcing for the first 60 hours and ECMWF IFS thereafter, and both run 4× daily at 00/06/12/18 UTC — a coarser cadence than HARMONIE's own 8× daily collection.
