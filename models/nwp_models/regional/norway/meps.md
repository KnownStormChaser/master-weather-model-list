...
- **Grid dimensions:** 949 × 1069 grid points at 2.5 km uniform spacing (verified from the THREDDS OPeNDAP `x`/`y` coordinate arrays: 949 × 1069 points at exact 2500 m spacing, ≈ 2370 × 2670 km). *(Both the deterministic and 30-member lagged products use this grid; if the previously stated 960 × 1080 referred to the internal computational/extension grid, it is not what the distributed files carry.)*
...
- **Vertical levels:** 65 (lowest level ~12 m above ground); a 90-level upgrade has been in preoperational testing since March 2024. As of the 2026-07-13 run the distributed operational product still carries 65 model levels (verified: `hybrid = 65` in the deterministic full-field product), so the 90-level rollout is not yet reflected in the data.
...
- **Alternative distributions:** The same MetCoOp MEPS production is also redistributed by FMI in a different access scheme and a reduced subset — deterministic fields only, on surface, nine isobaric levels, and thinned model levels, with no archive (see [HARMONIE (FMI)](../finland/harmonie-fmi.md)).
...
- **Companion suites — MNWC and MECaS.** MetCoOp also runs two operational products built on the same modelling grid as MEPS:
  - **MNWC (MetCoOp Nowcasting System):** A deterministic 12-hour forecast on the same 2.5 km MEPS grid (949 × 1069), refreshed hourly with a 25-minute observation cutoff and 15-minute output time resolution. MNWC immediately follows its main analysis with an additional cloud-initialization step ingesting geostationary satellite imagery (Gregow et al. 2020). Two MNWC instances run in parallel on Stratus and Vinha. Operational since 2022.
...
- **File naming convention:** Files follow `<model>_<form>_<resolution>_<DTG>.<type>`, where `<form>` is `det` for the control member, or `lagged_Xh_subset`/`lagged_Xh_latest` for time-lagged ensemble output. Most files in the latest catalogue are NCML-only and must be accessed via OPeNDAP rather than downloaded directly. In the current latest catalogue the only products offered as direct NetCDF are the time-lagged EPS `meps_lagged_6_h_latest_2_5km_*` files — the per-cycle `.nc` and the rolling `_latest.nc`, each ≈ 121 GB for the full 30-member set. `det`, `det_vc`, `subset`, and `lagged_6_h_vc` are NCML-only. *(Verified against the latest catalogue on 2026-07-13; `meps_det_vc_*` may have carried NetCDF previously but is currently NCML-only.)*
...
- **Bris — the AI model over the same domain.** MET Norway also runs [Bris](../../../ensemble_models/regional/norway/bris.md), a data-driven (machine-learning) ensemble distributed on essentially the MEPS grid (849 × 969, the MEPS domain less a 50-point perimeter band, same Lambert conformal projection). Anyone looking for AI-based forecasts over the MetCoOp area should start there. MEPS is central to it in two ways: Bris is **trained on historical MEPS analyses**, and MEPS output supplies part of the **initial conditions** for every Bris run, alongside ECMWF IFS. The practical consequence is that Bris is **not an independent forecast** relative to MEPS and should not be treated as one for verification, blending, or grand-ensemble purposes. Bris is ensemble-only — 16 members hourly to 90 h, 30 members 6-hourly to 258 h, with no deterministic run and no control member — so its main draw is a 2.5 km ensemble reaching ten days where MEPS reaches 61 hours. MET Norway labels it experimental, and it currently has no archive. Indexed in [`AI_MODELS.md`](../../../../AI_MODELS.md).
- **Relationship to siblings:**
  ...
  - [HARMONIE (FMI)](../finland/harmonie-fmi.md) — FMI's redistribution of a deterministic MEPS subset via a different access scheme (WFS + binary download service). FMI serves surface fields, nine isobaric levels, and model levels 12–65 on a 3×-thinned grid; it distributes no ensemble members, cross-sections, or archive.
  - [Bris](../../../ensemble_models/regional/norway/bris.md) — MET Norway's data-driven AI ensemble on effectively the same grid, trained on MEPS analyses and partly initialized from MEPS. See the dedicated note above.
