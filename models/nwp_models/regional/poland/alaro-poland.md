...
- **Update frequency / cycles:** 4× daily (00, 06, 12, 18 UTC) per the IMGW posters. **TBD against the live feed:** only three cycles were resident on 2026-08-07 (see *Retention and publication cadence* under Notes), which is consistent with 4× daily under a short rolling window but does not confirm it.
...
- **Precipitation, four separate accumulations:** large-scale precipitation (62), convective precipitation (63), large-scale snow (79), convective snow (78). **There is no single total-precipitation field** — total precipitation must be reconstructed as `62 + 63`.
...
- **Retention and publication cadence.** On 2026-08-07 04:00 UTC exactly three cycles were resident on the server — 2026-08-05 12 UTC, 2026-08-05 18 UTC and 2026-08-06 00 UTC — with `20260805_06` and earlier, and `20260806_06` and everything later, all returning 404. A portal screenshot taken at 2026-08-07 00:52 local shows the same three. Two readings are consistent with this snapshot and it cannot distinguish them: a ~3-cycle rolling window that has **stalled since the 2026-08-06 00 UTC cycle** (a ~28 h publication gap, i.e. an outage), or a publication cadence sparser than the 4× daily the posters describe. **Repeat observation needed** — this is a single snapshot and the retention depth (~3 cycles / ~36 h) is itself worth recording either way, since it is short enough to matter for anyone building a harvester.
...
- **AROME nest:** ALARO provides the lateral boundary conditions for [AROME (Poland)](./arome-poland.md) (LBC from ALARO-1 4 km). **The AROME entry currently carries the same "What it provides" field list as this entry did before verification, and the same unqualified "GRIB" format field — both are very likely wrong in the same ways and should be re-verified against `AROME_pub` files directly.**
