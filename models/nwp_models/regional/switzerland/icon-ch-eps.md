...
- **Parameter inventory:** 118 parameters for ICON-CH1-EPS (16 multi-level), 121 for ICON-CH2-EPS (18 multi-level), published as a CSV in each collection's STAC assets. The sets are **not** identical: `DBZ` and `SI` are ICON-CH1-EPS only; `DEN`, `DHAIL_MX`, `LPI_MAX` and the pollen fields are ICON-CH2-EPS only. The ICON-CH2-EPS counts are **season-dependent** — pollen fields are multi-level (`HFL`) and drop out of both the data and the CSV outside their flowering windows, so the totals fall to 119 / 16 multi-level when no species is in season. The figures above were counted on 2026-08-15 with grasses and ragweed active
...
- Both collections extended with additional **radiation, cloud, hydrology, soil, lake, visibility and diagnostic** variables. MeteoSwiss published only this category list, not a per-field inventory, and the release date was itself backfilled into the changelog on the day.
- The parameter counts recorded under **Data availability** already reflect this extension (re-verified against the live parameter CSVs, 2026-08-15).
- **Includes the orographic-shading radiation outputs foreshadowed in the March 2026 notice.** `ASWDIR_S_OS` and `ASWDIFU_S_OS` — direct and upward diffuse shortwave at the surface *including* orographic shading but *excluding* slope correction — are present in both collections. They appear in the parameter CSV and in live data but are **not described anywhere in the MeteoSwiss prose documentation**; the March 2026 ICON Info is the only source explaining their semantics.
  - **Encoding trap (verified by decoding, 2026-08-15):** `ASWDIR_S_OS` and `ASWDIR_S` are *indistinguishable by GRIB2 parameter identification* — both decode as discipline 0, `parameterCategory` 4, `parameterNumber` 198, product definition template 11, identical `stepRange`. They differ only in `typeOfFirstFixedSurface`: **208** (a local surface type; stock ecCodes reports `typeOfLevel = unknown`) for the `_OS` variant, against **1** (ground or water surface) for the standard field. Tooling that keys on parameter number alone will silently conflate the two.
  - Values behave as documented: in the 0–7 h average from the 2026-08-14 06 UTC ICON-CH1-EPS control run, `ASWDIR_S_OS` was less than or equal to `ASWDIR_S` at every one of the 1,147,980 cells, differing at 24,352 of them (2.1%), maximum reduction 259.3 W m⁻² — shading removes direct beam in shaded cells only.
- Other fields in the current inventory consistent with the listed categories: `VIS`; `CEILING`, `HBAS_SC`, `HTOP_SC`; `RUNOFF_S`, `RUNOFF_G`, `W_SNOW`, `RHO_SNOW`, `SNOWC`; `W_SO`, `W_SO_ICE`, `W_I`, `T_SO`; `T_MNW_LK`, `T_WML_LK`, `T_BOT_LK`, `C_T_LK`, `H_ML_LK`; `ALB_DIF`, `ALB_RAD`, `ASWDIFU_S`. *TBD: no pre-extension inventory snapshot exists, so this mapping is inferred from the category list rather than confirmed field by field.*

### 21 May 2026 — pollen added to ICON-CH2-EPS
...
- **Slope correction and orographic shading of shortwave radiation activated**, substantially cutting the overestimation of global radiation and sunshine duration in Alpine valleys (largest reduction in winter, present in all seasons) and the winter daytime warm bias there. The Swiss Plateau is virtually unaffected.
  - **Existing surface radiative flux output fields were unchanged in definition** — the corrections are applied only after those variables are written out. **`DURSUN` is the exception: its definition changed on this date.** Sunshine duration series spanning 24 March 2026 are not homogeneous.
- **Vertical turbulent mixing reduced over sloped terrain** — a slope-dependent reduction of both the near-surface minimum vertical diffusion coefficient and the laminar transfer resistance, enhancing nocturnal cooling. Substantially reduces the nocturnal Alpine-valley warm bias and the 2 m relative humidity bias; slightly degrades Alpine mountain stations, where an existing cold and moist bias grows.
- **KENDA-CH1 observation handling revised:** the artificial relative humidity saturation adjustment was removed (values above 96% were previously forced to 100%); surface stations without pressure measurements are now assigned model surface pressure at the nearest grid point instead of a standard-atmosphere estimate; SYNOP stations above 3000 m are excluded; sub-hourly SYNOP data are discarded; and a new LETKF version was introduced.
- A bug was fixed in the post-processing tool that interpolates output to the Swiss coordinate system (LV03 / LV95), so the nearest-neighbour point may differ from before at some locations. This affects the Swiss-grid post-processed products, not the native-grid GRIB2 distributed through Open Data.
- Announced first on 12 November 2025 for "Q1 2026", then re-issued on 12 March 2026 with the confirmed date; both PDFs remain linked from the changelog. MeteoSwiss reserves the right to postpone such introductions when severe weather warnings are in force, giving 24 h notice — which is why announced and effective dates can diverge.
