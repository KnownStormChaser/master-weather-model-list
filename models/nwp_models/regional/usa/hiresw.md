...
- **CONUS, Alaska, Hawaii, Puerto Rico:** scheduled for retirement **October 14, 2026 at 12 UTC** under NWS SCN 26-47 (termination notice, AAB update of September 9, 2026; RRFS/REFS implementation under companion SCN 26-48 AAD), originally proposed in NWS PNS 25-41 (June 26, 2025). The date moved from August 31 to October 6 in the July 6, 2026 update and from October 6 to October 14 in the September 9, 2026 update. Replaced by [RRFS](./rrfs.md) (CONUS/AK at 3 km; HI/PR at 2.5 km) and the [REFS](../../../ensemble_models/regional/usa/refs.md) ensemble. Subject to the standard CWD/ECE contingency.
- **Guam:** explicitly **preserved** — RRFS does not extend to the Guam domain, leaving HiresW as the operational convection-allowing guidance source for the western Pacific after the other domains retire. SCN 26-47 AAB scopes the withdrawal of the NOMADS tree as everything under `/pub/data/nccf/com/hiresw/prod/` **`EXCEPT *guam*`**, so the directory itself survives with only the Guam files in it. Code that enumerates the directory will keep working but will return a much smaller set; code that assumes a domain is present will not.
- **Withdrawn access points and headers.** The GRIB filter endpoints `gribfilter.php?ds=hiresconus`, `hiresak`, `hireshi` and `hirespr` end at retirement. Discontinued WMO headers are `[YZ][A-Z][AM][ABCDEFGHIKLOPQ][0-9][0-9] KWBS` (CONUS), `[YZ][A-Z]C[ABCDEFGHIKLOPQ][0-9][0-9] KWBS` (Alaska), `[YZ][A-Z]D[ABCDEFGHIKLOPQ][0-9][0-9] KWBS` (Hawaii) and `[YZ][A-Z]E[ABCDEFGHIKLOPQ][0-9][0-9] KWBS` (Puerto Rico).
- **Not everything HiresW produced is reproduced natively by RRFS.** EMC publishes three separate lists: HiresW variables not generated within RRFS (`hiresw_retirements.txt`), HiresW SBN products not provided by the RRFS SBN feed (`hiresw_sbn_removals.txt`), and a third file, `smartinit_products.txt`, covering products that SCN 26-47 says will be "generated by the Smartinit system processing RRFS output rather than directly generated by the RRFS."
- **⚠️ The Smartinit fallback described in SCN 26-47 no longer exists.** SmartInit was terminated in its entirety on **July 21, 2026 at 12 UTC** under [NWS SCN 26-59](https://www.weather.gov/media/notification/pdf_2026/scn26-59_SmartInit_Termination.pdf) (June 17, 2026), which removed `nomads.ncep.noaa.gov/pub/data/nccf/com/smartinit/` and withdrew the `[LM][ADEHKRSTUVZ][EHIK][A-Z][089][068] KWBE` WMO headers, directing users to the [NBM](./nbm.md) instead. SCN 26-47 nevertheless still points at Smartinit as the route by which certain HiresW products survive, through the AAB update of September 9, 2026 — **two months after the system it names was shut down.** The `smartinit_products.txt` file is more hedged than the SCN prose, listing fields that "should be possible to generate with an updated smartinit" rather than committing to it. Treat the products on that list as **discontinued** unless and until a later notice says otherwise, and plan on NBM as the replacement path. Note separately that the two probability fields on that list (3/6/12 h POP and 3 h probability of a wetting rain) are retired regardless, as consequences of the SREF retirement rather than of the Smartinit question.
- **HiresW model software is published as-is at retirement.** NCO has posted the final code at https://www.nco.ncep.noaa.gov/pmb/codes/nwprod/hiresw.v8.1.15/, deprecated, with no warranty, bug fixes, or support, and stated to be removed when the application retires — thereafter available only on request. Mirror it before October 14, 2026 if you need it. Note this covers the whole HiresW codebase including the surviving Guam configuration.
- **Caution on the `hiresw_retirements.txt` file:** it is live at two paths with different content. See [Notes](#notes).
...
- **EMC's HiresW transition files are live at two paths with different content.** SCN 26-47 AAB cites `https://www.emc.ncep.noaa.gov/rrfs_info/hiresw_retirements.txt`; an older copy sits at `https://www.emc.ncep.noaa.gov/mmb/mpyle/rrfs_info/hiresw_retirements.txt` and both return 200. The SCN-cited copy carries three extra lines — a "Planned" section listing PBL-height `HGT` and the Haines Index (HINDEX) as further retirements — that the `mmb/mpyle` copy omits. The same drift affects the NAM and HREF equivalents. Use the `/rrfs_info/` copies; they are the ones the current SCN points at and the newer of the two wherever they differ.
...
- NWS SCN 26-47, AAB update of September 9, 2026 (termination of CONUS/AK/HI/PR HiresW domains, Guam preserved; effective October 14, 2026): https://www.weather.gov/media/notification/pdf_2026/SCN26-47_Updated_Retire_NAM_SREF_HREF_HiresW_NAM_MOS.aab.pdf
- NWS SCN 26-48, AAD update of September 9, 2026 (RRFS/REFS implementation; effective October 14, 2026): https://www.weather.gov/media/notification/pdf_2026/scn26-048_Updated_RRFS_and_REFS_Implementation_aad.pdf
- HiresW variables not generated within RRFS (SCN-cited copy): https://www.emc.ncep.noaa.gov/rrfs_info/hiresw_retirements.txt
- HiresW SBN products not provided by the RRFS SBN feed: https://www.emc.ncep.noaa.gov/rrfs_info/hiresw_sbn_removals.txt
- HiresW products SCN 26-47 attributes to Smartinit — read alongside the SmartInit termination notice below: https://www.emc.ncep.noaa.gov/rrfs_info/smartinit_products.txt
- NWS SCN 26-59 (termination of all SmartInit products, effective July 21, 2026): https://www.weather.gov/media/notification/pdf_2026/scn26-59_SmartInit_Termination.pdf
- HiresW model software, deprecated and provided as-is: https://www.nco.ncep.noaa.gov/pmb/codes/nwprod/hiresw.v8.1.15/
