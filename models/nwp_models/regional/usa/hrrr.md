...
- **[RRFS](./rrfs.md) and [REFS](../../ensemble_models/regional/usa/refs.md):** Future replacement for HRRR and RAP. RRFSv1 implements **October 14, 2026 at 12 UTC** under SCN 26-48 AAD (the September 9, 2026 update, which moved the date from October 6) but does **not** retire HRRR — HRRR retirement is tied to the future RRFSv2 (MPAS-based) timeline. Notably, **HRRR contributes two members (current and 6 h old cycles) to the CONUS and Alaska REFS domains**, so HRRR is an explicit operational input to REFS from October 14, 2026 onward until HRRR itself is retired. The contribution is confirmed in the REFS data as well as the notice: `numberOfForecastsInEnsemble` decodes as 14 on the CONUS and Alaska REFS products against 12 on Hawaii and Puerto Rico, the difference being the two HRRR members.
...
- **HYSPLIT** — HRRR supplied two members to the regional-ensemble dispersion inputs as `hysplit.tCCz.rens.hrrr.m0M`. **This dependency ends October 14, 2026**: [NWS SCN 26-78](https://www.weather.gov/media/notification/pdf_2026/scn26-78_HYSPLIT_v9.1.pdf) (September 9, 2026) upgrades HYSPLIT to 9.1, couples RRFS in, and removes the `rens.hrrr`, `rens.nam` and `rens.hiresw` input files. Unlike NAM and HREF, HRRR itself is not retired — only this particular coupling is dropped. See the [HYSPLIT-Dust entry](../../air_quality_models/regional/usa/hysplit-dust.md).
...
HRRR is expected to be retired alongside [RAP](./rap.md) as part of the eventual transition to **RRFSv2** (the MPAS-based version of the [Rapid Refresh Forecast System](./rrfs.md)). Unlike the NAM, NAM Nest, HiresW (non-Guam), HREF, and SREF — which are scheduled for retirement on October 14, 2026 under [NWS Service Change Notice 26-47](https://www.weather.gov/media/notification/pdf_2026/SCN26-47_Updated_Retire_NAM_SREF_HREF_HiresW_NAM_MOS.aab.pdf) (termination notice, AAB update of September 9, 2026; RRFS/REFS implementation under companion [SCN 26-48](https://www.weather.gov/media/notification/pdf_2026/scn26-048_Updated_RRFS_and_REFS_Implementation_aad.pdf) AAD, same date; originally proposed under PNS 25-41) — **HRRR and RAP are explicitly not part of the RRFSv1 retirement wave**. Both are expected to continue operating in parallel with RRFSv1 until the RRFSv2 transition. SCN 26-48 also assigns HRRR an active role in the new REFS, contributing two members (current and 6 h old cycles) to the CONUS and Alaska domains — formalising HRRR as a continuing operational input rather than just a legacy system in maintenance.

No formal Service Change Notice for HRRR retirement has been issued as of September 2026 — confirmed against the full NWS notification index, which carries no HRRR termination notice in any year. The RRFSv1 date has now slipped three times (to August 31, then October 6, then October 14, 2026) without HRRR ever entering the retirement set, which is itself weak evidence that the RRFSv2 transition is not imminent.

**Watch the REFS dependency when HRRR retirement does arrive.** HRRR is not merely running in parallel with RRFSv1 — it supplies two of the fourteen members on the CONUS and Alaska REFS domains. Retiring HRRR therefore requires a REFS membership change, not just a decommissioning notice, so an HRRR retirement SCN should be expected to arrive alongside or after a REFS upgrade rather than on its own.
