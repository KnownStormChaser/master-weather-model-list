...
- **[RRFS](./rrfs.md):** Replacement. RRFSv1 implements October 14, 2026 at 12 UTC under SCN 26-48; it covers CONUS at 3 km, Alaska at 3 km, Hawaii at 2.5 km, and Puerto Rico at 2.5 km, providing functionally equivalent convection-allowing guidance to all NAM Nest domains. The 1.5 km fire-weather nest is replaced by a separate relocatable RRFS fire-weather run — described in the SCN as a 5° × 5° rotated lat-lon domain, but in fact Lambert conformal at 1270 m (see [Status and retirement timeline](#status-and-retirement-timeline)). Many NAM Nest output grids will continue to be produced from RRFS output via post-processing.
...
- **Scheduled for full retirement October 14, 2026 at 12 UTC** under NWS Service Change Notice 26-47 (termination notice; AAB update of September 9, 2026), as part of the broader NAM system retirement; RRFS/REFS implementation under companion SCN 26-48 (AAD update, same date). The date moved from August 31 to October 6 in the July 6, 2026 update, and from October 6 to October 14 in the September 9, 2026 update.
...
- The 1.5 km fire-weather nest is replaced by a relocatable RRFS fire-weather run, removing the gap previously noted in fire-weather moveable-nest coverage. **The SCN's description of that domain is wrong and should not be carried into this entry.** SCN 26-48 describes the RRFS fire-weather output as covering "a 5 x 5-degree rotated latitude longitude region" through the AAD update of September 9, 2026; decoding the live output shows a **Lambert conformal grid at 1270 m** (LoV 265°E, Latin1 = Latin2 = LaD = 25°N), roughly 663 × 495 km, not 5° × 5° and not rotated lat-lon. The filename token `1p5km` also overstates the increment. See the [RRFS entry](./rrfs.md#what-area-it-covers) for the decoded geometry and the observed two-placement alternation by cycle hour.
- Originally proposed for early 2026 under PNS 25-41; the date slipped through pre-operational evaluation of RRFSv1 before being scheduled in SCN 26-48. Per the SCN, if October 14, 2026 is declared a Critical Weather Day, an Enhanced Caution Event, or other significant weather is occurring or anticipated, retirement moves to 12 UTC on the next eligible weekday.
- Some NAM Nest output grids will continue to be produced from RRFS output via post-processing to ease the transition; others will be fully discontinued. EMC has published lists of retained vs. fully discontinued grids:
  - Retained NAM Nest output grids: https://www.emc.ncep.noaa.gov/mmb/mpyle/rrfs_info/namnest_grids.txt — note this file exists **only** under the `mmb/mpyle/` path and has no equivalent under the `https://www.emc.ncep.noaa.gov/rrfs_info/` path that SCN 26-47 AAB otherwise cites. Where a file exists at both paths the two copies have drifted; see the [NAM entry](./nam.md#status-and-retirement-timeline).
  - NAM fields not generated by RRFS (SCN-cited copy, covers the nests): https://www.emc.ncep.noaa.gov/rrfs_info/nam_retirements.txt
...
- The 1.5 km fire weather nest is one of the relatively few operational moveable-domain NWP forecasts globally — most operational systems use fixed domains. This makes it operationally distinctive. Under SCN 26-48, the equivalent capability is established in RRFSv1's relocatable fire-weather run — meaning the moveable-nest fire-weather guidance transitions directly from NAM Nest to RRFS on October 14, 2026 rather than leaving an operational gap. One caveat on equivalence: the NAM Nest fire-weather domain was repositioned each cycle at locations chosen for operational need, whereas the RRFS fire-weather domain has been observed taking only two placements, strictly alternating by cycle hour, across a twelve-day sample. Whether it repositions in response to fire-weather conditions is unresolved — see the [RRFS entry](./rrfs.md#what-area-it-covers).
