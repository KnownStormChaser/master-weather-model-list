...
- **Relationship to other MET Norway marine systems.** NorKyst v3 is the current-forcing source for [MyWaveWAM800m](../../../wave_models/regional/norway/mywavewam800m.md). Its closest relative is [Norshelf (NORKYST_DA)](./norshelf.md) — the same domain and projection at 2.4 km (an exact 3:1 resolution ratio), running ROMS 4.3 with 4D-Var assimilation. Norshelf is the plausible source of the analysis that this system's `norkyst_ana_nud-0.5.nc` nudges toward, and the plausible source of its lateral boundaries, but **neither system's metadata names the other and both linkages are unverified**. MET Norway's `fou-hi` catalog also carries **Topaz5**, **Barents-2.5 km (ROMS EPS)**, and **AICE** — not yet documented in this repository.
