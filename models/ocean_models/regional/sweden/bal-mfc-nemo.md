...
- **Reciprocal boundary exchange with the North West Shelf.** This product takes its western lateral boundaries from the Copernicus Marine NWS physics forecast, cited in the PUM as `NWSHELF_ANALYSISFORECAST_PHY_004_013`. **The dependency runs both ways**: both NWS PUMs name *this* product as the source of their own Baltic boundary (T, S, barotropic u and v). Neither side's documentation mentions the return path — it only surfaces from reading both. This is a mutual dependency, not a nesting hierarchy, so neither product is the other's parent.

  **Which NWS configuration actually supplies the boundary is ambiguous.** The BAL PUM cites only the `004_013` identifier, but the Met Office now runs two NWS physics configurations — [AMM15](../uk/amm15-nws.md) at 1.5 km and [AMM7](../uk/amm7-nws-lr.md) at 7 km — and both name the Baltic as a source in the other direction. The larger AMM7 domain is the more natural boundary provider for a Baltic model, but the PUM does not say (*TBD*).

  The identity of `004_013` with the Met Office AMM15 system is now **confirmed** — see the [AMM15 entry](../uk/amm15-nws.md), which verifies it from the PUM's own file-naming convention. The BAL PUM also notes the boundary source "will change production system again from November 2025", which matches the Met Office re-introduction documented there and in the [AMM15-WW3](../../../wave_models/regional/uk/amm15-ww3-uk.md) entry.
...
- **The PUM's swapped de-tided standard names are a documentation error only — but the same swap reaches the data in IBI.** This product's live files are correctly labelled. [IBI](../spain/ibi-phy-005-001.md) carries the identical swap **in the delivered files**, and the [Black Sea](../italy/blk-phy-007-001.md) omits de-tided labelling altogether, using plain `uo`/`vo`/`zos` with no `_assuming_no_tide` standard name. Four Copernicus regional physics products, four de-tided conventions. Code written against one will not transfer.
