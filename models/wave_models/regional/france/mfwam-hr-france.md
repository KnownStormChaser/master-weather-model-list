...
- **Relationship to the WW3 coastal packages.** Météo-France also publishes [WW3-MARO](./ww3-maro-france.md), [WW3-MARP](./ww3-marp-france.md) and [WW3-WARP](./ww3-warp-france.md) under the same `pnt-vague-surcote` portal tag. Those are WAVEWATCH III systems on **unstructured meshes** distributed as **NetCDF**, and they cover overlapping waters with genuine coastal shallow-water physics that this product lacks — worth preferring for nearshore work. They are present in the object store, but publish about 20 minutes later than these GRIB2 files, so a listing taken during the in-progress cycle will not show them. There is **no WW3-WARO**; the four-way MARO/MARP/WARO/WARP naming applies only to the HYCOM2D surge packages.
